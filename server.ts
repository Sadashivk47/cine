...
import { CURATED_MOVIES } from "./src/data";
  ...
  try {
    ...
    try {
      ...
      if (query) {
        ...
        const data = await fetchFromTMDB(endpoint, { query, page });
      ...
      console.warn("TMDB fetch bypassed, falling back to local records:", err.message);
    }
    ...
    console.error("Express /api/movies query handler failed:", globalErr);
    res.status(500).json({ movies: [], hasMore: false, totalResults: 0, isTMDB: false });
  }
});
...
app.get("/api/movies/featured", async (req, res) => {
  try {
    try {
      ...
      console.warn("TMDB featured movie query failed, falling back to local list:", err.message);
    }

    const featured = CURATED_MOVIES.find(m => m.isFeatured) || CURATED_MOVIES[0];
    res.json(featured);
  } catch (globalErr: any) {
    console.error("Express /api/movies/featured handler failed:", globalErr);
    const featured = CURATED_MOVIES.find(m => m.isFeatured) || CURATED_MOVIES[0];
    res.json(featured);
  }
});
...
app.get("/api/movies/:id/trailer", async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn(`Failed to resolve live TMDB trailer videos for ${fullId}:`, e.message);
    }

    res.json({ videoId: "u48_JpUloGY" });
  } catch (globalErr: any) {
    console.error("Express /api/movies/:id/trailer handler failed:", globalErr);
    res.json({ videoId: "u48_JpUloGY" });
  }
});
